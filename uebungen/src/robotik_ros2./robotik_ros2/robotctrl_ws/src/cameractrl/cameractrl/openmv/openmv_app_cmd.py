...
import sensor, image, time, sys, binascii
...
# led_red = LED("LED_RED")          # Or red_led = LED(1)
led_green = LED("LED_GREEN")      # Or green_led = LED(2)
led_blue = LED("LED_BLUE")        # Or blue_led = LED(3)
...
def receive_file_simple(default_name='test.py'):
...
def receive_file(default_name='test.py'):
    try:
        ...
        if name_len == 0:
            ...
            fname = io.read_exact(name_len).decode()
        # Gesamtlänge
        total_size = int.from_bytes(io.read_exact(4), 'big')

        # Datei öffnen
        path = '/flash/{}'.format(fname)
        f = open(path, 'wb')

        # Chunks empfangen, bis total_size erreicht
        received = 0
        crc = 0
        try:
            while received < total_size:
                hdr = io.read_exact(2)            # 2-Byte Chunk-Länge (BE)
                clen = io.u16_be(hdr)
                if clen == 0:
                    io.write(NAK)  # ungültig
                    continue
                data = io.read_exact(clen)
                # schreiben
                f.write(data)
                received += clen
                # CRC fortschreiben
        ...
        # Erwartete CRC lesen und vergleichen
        expected_crc = int.from_bytes(io.read_exact(4), 'big')
